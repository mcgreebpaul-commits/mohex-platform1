...
export const submitTrade = async (req: AuthenticatedRequest, res: Response) => {
  const userId = req.user?.id;
  ...
  try {
    ...
    const [balanceResult]: any = await connection.execute('SELECT balance FROM portfolios WHERE user_id = ? FOR UPDATE', [userId]);
    ...
    const [tradeResult]: any = await connection.execute(
...
export const submitDepositRequest = async (req: AuthenticatedRequest, res: Response) => {
  const userId = req.user?.id;
