...
const connection: any = await (db as any).getConnection();
try {
    ...
    await connection.execute(
      `INSERT INTO subscriptions (user_id, plan_id, status, expires_at) VALUES (?, ?, 'active', DATE_ADD(NOW(), INTERVAL 1 YEAR))`,
      [newUserId, 1]
    );
